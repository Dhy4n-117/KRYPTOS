...
const HashEngine = {
  async hashAll(text) {
    ...
    for (const algo of ['SHA-1','SHA-256','SHA-384','SHA-512']) {
      ...
      results[algo.toLowerCase().replace('-','')] = arr.map(b=>b.toString(16).padStart(2,'0')).join('');
    }
    // MD5 (pure JS implementation)
    results.md5 = this.md5(text);
    // CRC32
    results.crc32 = this.crc32(text);
    return results;
  },
  ...
  md5(string) {
    ...
    for(i=0;i<4;i++)for(let j=0;j<4;j++)s+=hex[(state[i]>>(j*8+4))&0xF]+hex[(state[i]>>(j*8))&0xF];
    return s;
  },

  // CRC32
  crc32(str) {
    let table = HashEngine._crc32Table;
    if (!table) {
      table = []; let c;
      for (let n = 0; n < 256; n++) { c = n; for (let k = 0; k < 8; k++) c = c & 1 ? 0xEDB88320 ^ (c >>> 1) : c >>> 1; table[n] = c; }
      HashEngine._crc32Table = table;
    }
    let crc = 0xFFFFFFFF;
